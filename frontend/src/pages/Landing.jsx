...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen sky-gradient overflow-hidden relative" data-testid="landing-page">
      ...
      {/* Footer */}
      <div className="relative lg:absolute lg:bottom-4 lg:left-0 lg:right-0 mt-10 pb-28 lg:pb-0 text-center text-sky-500 text-sm">
      ...
      {/* Sticky mobile CTA — keeps the primary action reachable without
          scrolling back to the hero. Desktop already has room for the
          in-hero buttons, so this only shows below the lg breakpoint. */}
      <div
        className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-white/90 backdrop-blur-md border-t border-sky-100 px-4 pt-3"
        style={{ paddingBottom: "calc(0.75rem + env(safe-area-inset-bottom))" }}
      >
        <Button
          size="lg"
          className="w-full rounded-full bg-sky-500 hover:bg-sky-600 text-white shadow-lg text-base py-6"
          onClick={() => navigate('/signup')}
          data-testid="sticky-get-started-btn"
        >
          <Play className="w-5 h-5 mr-2" />
          Get Started Free
        </Button>
